...
amazonController.createProductsObjFromHtml = ($) => {
    let products = [];
        for (let i = 0; i < 5; i++) {
            let keywords = $('#s-result-count span span.a-color-state.a-text-bold').text().replace('"','').replace('"' ,'');
            ...
            let stars = $('#result_' + i + ' .a-icon-alt').last().text();
            let reviews = $('#result_' + i + ' .a-size-small.a-link-normal.a-text-normal').text();

            let product  = {keywords, name, asin, url, imageUrl, manufacturer, price, stars, reviews};
